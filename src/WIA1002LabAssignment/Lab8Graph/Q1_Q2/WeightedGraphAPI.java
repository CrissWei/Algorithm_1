...
public class WeightedGraphAPI<T extends Comparable<T>, N extends Comparable<N>>{
    Vertex<T, N> head;
    int size;

    public WeightedGraphAPI(){
        head = null;
        size = 0;
    }
    ...
    public boolean addVertex(T v){
        if(!hasVertex(v)){ // vertex is not in the graph
            Vertex<T, N> pointer = head;
     ...
     * @param weigh the weight of the edge, if desired
     * @return true if successfully added, false if not successful
     */
    public boolean addEdge(T source, T destination, N weigh){
        if(head == null)
            return false;
            ...
            if(sourceVertex.vertexInfo.compareTo(source) == 0){
                ...
                while(destinationVertex != null){
                    if(destinationVertex.vertexInfo.compareTo(destination) == 0){
                        ...
                        Edge<T, N> currentEdge = sourceVertex.firstEdge;
                        Edge<T, N> newEdge = new Edge<>(destinationVertex, weigh, currentEdge);
